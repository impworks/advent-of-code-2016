...
class Solver
{
    ...
    private State[] GetChildStates(State state)
    {
        ...
        for (var i = 0; i < _steps.Length; i++)
        {
            ...
            newStates[i] = new State
            {
                ...
                Position = new Point(state.Position.X + step.Direction.X, state.Position.Y + step.Direction.Y)
            };
        }
    ...
    {
        var inputLength = _initialState.HashableString.Length;
        return state.HashableString.Substring(inputLength);
    }
