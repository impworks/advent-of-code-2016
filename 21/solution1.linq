...
void Main()
{
	...
	var result = scrambler.Scramble(lines);
...
class Scrambler
{
	public Scrambler(string source)
	{
		...
		_commands = new[]
		{
			...
			new Command
			{
				...
				Callback = (data, args) => Rotate(data, int.Parse(args["dist"]), args["dir"] == "left")
			},
			new Command
			{
			...
			new Command
			{
				...
				Callback = (data, args) => Move(data, int.Parse(args["from"]), int.Parse(args["to"]))
			},
		};
	}
	...
	public string Scramble(string[] lines)
	{
	...
	private void RotateByChar(char[] data, char ch)
	{
		...
		var dist = idx + 1 + (idx >= 4 ? 1 : 0);
		Rotate(data, dist, left: false);
	}
